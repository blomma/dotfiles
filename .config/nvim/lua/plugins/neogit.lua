return {
    ...
    dependencies = {
        ...
        { "AstroNvim/astroui", opts = { icons = { Neogit = "󰰔" } } },
        {
            ...
            opts = function(_, opts)
                ...
                maps.n[prefix .. "n"] = {
                    desc = require("astroui").get_icon("Neogit", 1, true)
                        .. "Neogit",
                }
                maps.n[prefix .. "nt"] =
                    { "<Cmd>Neogit<CR>", desc = "Open Neogit Tab Page" }
                maps.n[prefix .. "nc"] = {
                    "<Cmd>Neogit commit<CR>",
                    desc = "Open Neogit Commit Page",
                }
                maps.n[prefix .. "nd"] =
                    { ":Neogit cwd=", desc = "Open Neogit Override CWD" }
                maps.n[prefix .. "nk"] =
                    { ":Neogit kind=", desc = "Open Neogit Override Kind" }
                maps.n[prefix .. "nf"] = {
                    "<Cmd>Neogit kind=floating<CR>",
                    desc = "Open Neogit Float",
                }
                maps.n[prefix .. "nh"] = {
                    "<Cmd>Neogit kind=split<CR>",
                    desc = "Open Neogit Horizontal Split",
                }
                maps.n[prefix .. "nv"] = {
                    "<Cmd>Neogit kind=vsplit<CR>",
                    desc = "Open Neogit Vertical Split",
                }
            end,
        },
    },
    ...
    opts = function(_, opts)
        ...
        local disable_builtin_notifications = utils.is_available "nvim-notify"
            or utils.is_available "noice.nvim"
        if utils.is_available "snacks.nvim" then
            local snacks_notifier = utils.plugin_opts("snacks.nvim").notifier
            if
                snacks_notifier
                and vim.tbl_get(snacks_notifier, "enabled") ~= false
            then
